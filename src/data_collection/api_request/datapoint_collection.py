...
class DataPointCollection():
    ...
    def get_six_pre(self):
        ...
        # convert Timestamp
        df_six_avg = self.df_timestamp_floor(df_six_avg)
    ...
    def get_mskix_pre(self):
        ...
        # floor Timestamps
        df_mskix_avg = self.df_timestamp_floor(df_mskix_avg)
        df_mskix_max = self.df_timestamp_floor(df_mskix_max)
    ...
    def get_ficix(self):
        ...
        # floor Timestamps
        data_ficix = self.df_timestamp_floor(data_ficix)
    ...
    def create_date(self, concat):
        ...
        if concat:
            data_pee = self.get_peering_cz()
            data_linx = self.get_linx()
            data_mkix_avg, data_mkix_max = self.get_mskix_pre()
            data_six_avg, data_six_max = self.get_six_pre()
            data_ficix = self.get_ficix()
            data_all = pd.concat([data_linx, data_pee, data_mkix_avg, data_mkix_max, data_six_avg,
                                  data_six_max, data_ficix], ignore_index=True)

            self.save_dataframe(data_all, 'all', RESOURC_PATH)
        else:
            self.get_peering_cz()
            self.get_linx()
            self.get_mskix_pre()
